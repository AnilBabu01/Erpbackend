...
const GetExpensesAnalysis = async (req, res) => {
  try {
    ...
    let allexpenses = await sequelizes.query(
      `Select Expensestype,PayOption,SUM(ExpensesAmount) AS total_paidamount FROM expenses WHERE ClientCode= '${req.user?.ClientCode}' AND Expensestype IN ('Expenses', 'Liability') AND MONTH(Date) ='${month}'  AND Session ='${sessionname}' AND PayOption IN ('Cash', 'Online')  GROUP BY Expensestype ,PayOption;`,
      {
        nest: true,
